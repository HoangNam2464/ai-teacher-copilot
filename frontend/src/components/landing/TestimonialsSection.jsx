...
function TestimonialCard({ tKey }) {
  const { t } = useTranslation();
  const globalIndex = testimonialKeys.indexOf(tKey);
  ...
  return (
    <div className="relative bg-card/80 backdrop-blur-sm border border-border hover:border-emerald-500/20 rounded-2xl p-6 lg:p-8 flex flex-col overflow-hidden group transition-colors duration-300 hover:shadow-md h-full">
      <Quote className="w-8 h-8 text-emerald-500/20 mb-4 shrink-0" />

      <p className="text-muted-foreground leading-relaxed flex-1 mb-6">
        &ldquo;{t(`testimonials.${tKey}.text`)}&rdquo;
      </p>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">
            {t(`testimonials.${tKey}.role`)} · {t(`testimonials.${tKey}.school`)}
          </p>
        </div>
      </div>
    </div>
...
export function TestimonialsSection() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      const w2 = measureCardWidth();
      x.set(-((offsetBase + currentIndex) * (w2 + gap)));
    };
    ...
    <section className="py-20 lg:py-32 relative overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10">
        ...
        <motion.div
          ...
          className="absolute top-1/3 -left-1/4 w-[500px] h-[500px] bg-gradient-to-br from-emerald-400/10 to-green-400/10 rounded-full blur-3xl"
        />
        <motion.div
          animate={{ opacity: [0.15, 0.25, 0.15], scale: [1.1, 1, 1.1] }}
          transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
          className="absolute bottom-1/3 -right-1/4 w-[400px] h-[400px] bg-gradient-to-br from-teal-400/10 to-emerald-400/10 rounded-full blur-3xl"
        />
      </div>
      ...
      <div className="container mx-auto px-8 sm:px-12 lg:px-16 xl:px-24">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          >
            <motion.span
              className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-sm font-medium mb-4"
