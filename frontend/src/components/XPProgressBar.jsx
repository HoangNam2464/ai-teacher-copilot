    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <div
            className={`w-11 h-11 rounded-xl bg-gradient-to-br ${levelInfo.gradient} flex items-center justify-center shadow-lg ${levelInfo.shadow}`}
          >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <span className={`text-xs font-semibold ${levelInfo.text}`}>
              {t(`gamification.levelNames.${stats.level}`, { defaultValue: levelInfo.name })}
            </span>
            <span className="text-xs text-muted-foreground">
              {xpInLevel} / {xpNeeded} {t('gamification.xp', 'Điểm')}
            </span>
          </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500/10 rounded-full">
          ...
          <span className="text-sm font-semibold text-orange-600 dark:text-orange-400">
            {stats.streakDays}{' '}
            {stats.streakDays === 1
              ? t('gamification.day', 'ngày')
              : t('gamification.days', 'ngày liên tục')}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <div className="text-xs">
            ...
            <span className="text-muted-foreground">
              {' '}
              / {stats.dailyGoal} {t('gamification.dailyXp', 'Mục tiêu hôm nay')}
            </span>
          </div>
        </div>
      </div>
